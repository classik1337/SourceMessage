...
export async function GET(request) {
  ...
  try {
    ...
    const [incomingRows] = await connection.execute(
      'SELECT f.*, u.avatar_url, u.second_name, u.full_name FROM messengerapp.friends f LEFT JOIN messengerapp.user_profiles u ON f.user_id = u.user_id WHERE f.friend_id = ? AND f.status = ? ORDER BY f.created_at DESC',
      [userId, 'pending']
    );
    // 6. Исходящие заявки (pending, user_id = мой id)
    const [outgoingRows] = await connection.execute(
      'SELECT f.*, u.avatar_url, u.second_name, u.full_name FROM messengerapp.friends f LEFT JOIN messengerapp.user_profiles u ON f.friend_id = u.user_id WHERE f.user_id = ? AND f.status = ? ORDER BY f.created_at DESC',
      [userId, 'pending']
    );
